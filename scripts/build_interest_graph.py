...
def run_inference(provider: str = "codex") -> dict:
    ...
    result_file = run_root / "result.validated.json"
...
def _legacy_store_for_recovery(_parsed: dict) -> int:
    """Retired unsafe writer retained only as an explicit recovery marker.

    All inference persistence must pass through the contract driver's
    validate-then-transaction path.  Keeping a callable legacy writer here
    would make the stable entrypoint expose an unvalidated graph mutation.
    """
    raise RuntimeError(
        "legacy unvalidated store is retired; use the contract driver")
...
# The earlier local implementation is retained above for owner recovery and
# review. Runtime and import callers use the reconciled contract driver below,
# which preserves strict validation, full-coverage bootstrap, reconciliation,
# and grounded-source integration. The contract allocates
# `_new_run_dir("recon")` for reconciliation prompt isolation.
from scripts import build_interest_graph_contract as _contract_driver  # noqa: E402

globals().update({
    name: value for name, value in vars(_contract_driver).items()
    if not name.startswith("__")
})

_CONTRACT_RUN_RECONCILIATION_TREE = _contract_driver.run_reconciliation_tree


def _sync_contract_test_seams() -> None:
    """Keep active-module monkeypatch seams compatible with the bridge."""
    _contract_driver._invoke_and_extract = _invoke_and_extract
    _contract_driver.ARTIFACT_ROOT = ARTIFACT_ROOT
    _contract_driver.validate_inference = validate_inference
    active_reconcile = globals().get("run_reconciliation_tree")
    if active_reconcile is _BRIDGE_RUN_RECONCILIATION_TREE:
        _contract_driver.run_reconciliation_tree = (
            _CONTRACT_RUN_RECONCILIATION_TREE)
    else:
        _contract_driver.run_reconciliation_tree = active_reconcile


def run_inference(*args, **kwargs):
    _sync_contract_test_seams()
    return _contract_driver.run_inference(*args, **kwargs)


def run_batch_inference(*args, **kwargs):
    _sync_contract_test_seams()
    return _contract_driver.run_batch_inference(*args, **kwargs)


def run_reconciliation_tree(*args, **kwargs):
    _sync_contract_test_seams()
    return _contract_driver.run_reconciliation_tree(*args, **kwargs)


_BRIDGE_RUN_RECONCILIATION_TREE = run_reconciliation_tree


def run_bootstrap(*args, **kwargs):
    _sync_contract_test_seams()
    return _contract_driver.run_bootstrap(*args, **kwargs)


def main(*args, **kwargs):
    _sync_contract_test_seams()
    return _contract_driver.main(*args, **kwargs)
