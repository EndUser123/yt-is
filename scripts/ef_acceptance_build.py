...
def build_auto():
    ...
    (BENCH / "acceptance_c1_auto.json").write_text(payload, encoding="utf-8")
...
def main() -> int:
    ...
    if a.mode == "auto":
        ...
        h = ""
        for f in ("acceptance_c1_auto.json", "acceptance_c1_hand.json"):
            p = BENCH / f
            if p.exists():
                h += hashlib.sha256(p.read_bytes()).hexdigest()[:16] + " "
        (BENCH / "acceptance_c1_seal.txt").write_text(
