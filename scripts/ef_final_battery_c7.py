...
def main() -> int:
    ...
    c5 = json.loads((BENCH / "shard01_auto.json").read_text(encoding="utf-8"))
    hand = json.loads((BENCH / "shard01_hand.json").read_text(encoding="utf-8"))
    lat: list[float] = []
    import sqlite3
    ...
    routed_s = all(routing.classify(h["query"]).intent in ("semantic",
                                                           "ambiguous")
                   for h in plain)  # contract: conventional terms route
                   # semantic OR the dual-retrieval ambiguous lane
    ...
    (BENCH / "c7_judging_listing.json").write_text(
    ...
    out = REPO / "docs" / "evidence-fabric" / "c7_battery_phase1.json"
    ...
    print(f"[c7] phase-1 receipt -> {out}")
    return 0
