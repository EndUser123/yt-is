...
def main() -> int:
    ...
    c5 = json.loads((BENCH / "shard03_auto.json").read_text(encoding="utf-8"))
    hand = json.loads((BENCH / "shard03_hand.json").read_text(encoding="utf-8"))
    lat: list[float] = []
    import sqlite3
    ...
    import json as _j
    auto2 = _j.loads((BENCH / "shard03_auto.json").read_text(encoding="utf-8"))
    ...
    g["ambiguous_allcaps_conventional"] = {"disc@10": round(conv_disc / 3, 4),
                                          "pass": conv_disc == 3.0}
    # v1.2: BEHAVIORAL gate for conventional terms (route unconstrained)
    import sqlite3 as _s2
    conv_terms = ["TikTok", "YouTube", "OpenAI"] +         [h["query"] for h in hand if h["stratum"] == "ambiguous_common"]
    disc_ok = judged_terms = 0
    for t in conv_terms:
        res = q.relevant(t, limit=10)
        m = routing.sanitize_fts_query(t)
        f2 = _s2.connect(f"file:{routing.FTS_DB}?mode=ro", uri=True)
        disc = any(f2.execute("select 1 from chunks where chunk_id=? and "
                              "chunks match ?", (r.chunk_id, m)).fetchone()
                   for r in res)
        f2.close()
        disc_ok += 1.0 if disc else 0.0
        judged_terms += 1
    g["conventional_behavioral"] = {
        "n": judged_terms, "literal_discoverable": round(disc_ok / judged_terms, 4),
        "pass": disc_ok / judged_terms >= 0.9}
    ...
    out = REPO / "docs" / "evidence-fabric" / "c9_battery_phase1.json"
    ...
    print(f"[c9] phase-1 receipt -> {out}")
    return 0
