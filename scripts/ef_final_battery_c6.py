...
def main() -> int:
    ...
    c5 = json.loads((BENCH / "acceptance_c6_auto.json").read_text(encoding="utf-8"))
    hand = json.loads((BENCH / "acceptance_c6_hand.json").read_text(encoding="utf-8"))
    lat: list[float] = []
    import sqlite3
    ...
    g["weak_common_routed"] = {"terms": weak_common, "routed_ambiguous": routed,
                               "pass": routed}
    plain = [h for h in hand if h["stratum"] == "ambiguous_plain"]
    routed_s = all(routing.classify(h["query"]).intent == "semantic"
                   for h in plain)
    ...
    listings = {}
    authored = {}
    for strat in ("telegraphic", "semantic_technical",
                  "comparison_questions"):
        items = [h for h in hand if h["stratum"] == strat]
        if strat == "comparison_questions":
    ...
    (BENCH / "c6_judging_listing.json").write_text(
        json.dumps(listings, indent=1, ensure_ascii=False), encoding="utf-8")
    for strat in ("telegraphic", "semantic_technical", "comparison_questions"):
    ...
    out = REPO / "docs" / "evidence-fabric" / "c6_battery_phase1.json"
    ...
    print(f"[c6] phase-1 receipt -> {out}")
    return 0
