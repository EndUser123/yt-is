...
- `sweep_phase3_2lane_3w_run06` then recovered the browser-health and auth state but still completed only at `2284.56`, with `794/6/800` and `browser_health.status="recovered_clean"` after reaping `12` default NotebookLM Chrome profile processes. That is an improvement over run05, but it remains far below the `4123.28` leader, so the remaining gap is not explained by auth TTL or browser-health hygiene alone.
...
- Operationally after run06, pivot away from auth-check cache TTL and browser-health hygiene and inspect source-add/readiness/setup cost or another non-TTL limiter.
...
Run05 completed and was negative, and the later guarded rerun `run06` stayed well below the historical `3+3` leader even after auth repair and browser-health hardening. Do not rerun this packet as-is. The next follow-on should pivot to source-add/readiness/setup cost or another non-TTL limiter.
