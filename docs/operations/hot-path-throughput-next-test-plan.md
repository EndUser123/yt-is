...
- **Candidate 6 per-attempt telemetry is live-proven (2026-07-01, run02).** The 11-field `nlm_batch_source_content_fetch_completed` contract validated on `candidate6_telemetry_validation_run02_current` (Signals 1/2/3 PASS on 1070 events; source-age-cliff `in_progress` leak fixed 169 → 0; per-attempt reconciliation = 0.0000 across 660 events — NOTE: tautological by construction, not proof of zero overhead). The measured distribution ranks the retry-tail Candidates 1-5 as minority levers. NOTE: the ranking packet's headline "primary-materialization dominates (median 57.5s)" is a metric artifact, corrected in [`.logs/sharded_lane_series/design_packet_primary_batch_wait_run02_mechanism_no_patch_current.md`](P://packages/yt-is/.logs/sharded_lane_series/design_packet_primary_batch_wait_run02_mechanism_no_patch_current.md): `primary_batch_wait_time_s` is a fetch-task start-age clock, not an independent backend materialization metric; completed-row `source_ready_age_s` can later advance to attempt/final age, so treating these fields as independent causal corroboration was invalid. The 57.5s median mixes retry-pass rows (primary-only median is 12.3s in the offline re-derivation). The current no-patch packet infers auth-churn-driven `_run_cmd` iteration inflation (`_ensure_nlm_auth` blocking; run02 had 926 auth events) slot-hogging the 10-wide fetch pool and cascading queued sources past the cliff; that inference still needs the planned cross-corpus overshoot/auth correlation before it should authorize a live benchmark. Next effort routes to durable auth (#965), not primary-materialization and not the retry tail. See [`.logs/sharded_lane_series/candidate6_mechanism_ranking_after_run02.md`](P://packages/yt-is/.logs/sharded_lane_series/candidate6_mechanism_ranking_after_run02.md) for the historical ranking. Do not open a Candidate 1-5 live benchmark on this evidence; the retry path is too small a contributor to be cost-justified. Analyzer: `python scripts/analyze_candidate6_smoke.py --run-root <run02>`.
...
Expected: no shared/default NotebookLM auth browser, no direct sibling login, and no browser asking for user action. A bounded hidden/headless browser tied to the configured Pro or Free CDP root is allowed only for worker-01 renewal. Any sign-in/challenge target, unprofiled force-login, or visible prompt invalidates the preflight.
...
- `nlm_batch` now guards the local source-content retry queue against projected source-age cliffs. If a failed source-content fetch is otherwise locally retry-queue deferrable, the local drain sleeps only the remaining safe headroom before retrying instead of always sleeping the full configured queue delay; if no safe headroom remains, the item is not queued for the local drain and the fetch-completed event records `retry_queue_skipped_reason=projected_source_age_cliff` plus `projected_retry_ready_age_s`. Shared retry-pool handoff remains eligible because the receiving worker re-adds and re-materializes in another notebook. Projected retry-age fields are now emitted only for actual retry-queue candidates; retry-pass rows no longer invent a second projection when retry queuing is disabled for that pass. The stage reducer surfaces both `Max Projected Retry Age(s)` and `Retry Queue Skipped` in the fetch recovery table, suppresses stale retry-pass projection fields from older traces, and now renders missing projected-age fields as `absent` rather than `0.0`, so the next artifact can show whether the guard fired without implying false zero-age projections. This is a code-path fix for the retry rows above, not a throughput datapoint; do not run a live validation on hotel Wi-Fi unless the goal is environment-scoped diagnostics. The exact-threshold boundary and actual-drain boundary are now covered by regression tests so the cliff remains inclusive.
- The local retry projected-age guard now also has an opt-in margin knob: `YTIS_NLM_SOURCE_CONTENT_RETRY_QUEUE_AGE_MARGIN_S` defaults to `0` through shared `NLMConfig.source_content_retry_queue_age_margin_s` and does not change current behavior unless explicitly set. When set, the same margin is subtracted from local drain headroom before the retry sleeps, so queued retries can preserve a buffer before the cliff or skip locally with `retry_queue_skipped_reason=projected_source_age_cliff_margin` if no margin-adjusted headroom remains; fetch attribution records both `retry_queue_age_margin_s` and `projected_retry_ready_age_with_margin_s`, and the reducer/audit reports surface max margin-adjusted projected age plus max configured margin. This exists to test the current retry-window hypothesis without another code change once home-network validation is available; it is not a validated throughput improvement yet.
...
The fresh run05 rerun does not change that conclusion. `fresh_state_3plus3_extract_schema_source_age_cadence_first_window_post_scope_fix_run05_current` stopped at `blocked_before_soak` because browser health was degraded and `unexpected_process_budget_exceeded=true` with `111` unexpected Chrome processes; smoke still reached `2929.41` combined hot-path VPH on `792/8/800`, with the residual misses split across ordinary `command_failed` counts (`3` Pro, `5` Free). The lane sweep summaries show the first-window cap was effectively inactive in this rerun (`source_age_cadence_first_window_size=0`, `window_count=0`, `active_window_count=0`, `extract_window_count=0`), so treat it as browser-health/process-budget evidence rather than a new cap result.
...
`fresh_state_3plus3_extract_schema_source_age_cadence_local_retry_projection_run06_current` reran the same shape after fixing dead-notebook recovery so it recreates an empty notebook before the caller's single retry. The target failure did not recur: soak contained zero `nlm_batch_source_mapping_failed` events, and recovered workers no longer showed run05's duplicated `50 -> 100` source-count sequence. Smoke was complete at `3290.59` combined VPH with `791/9/800`. Soak is still not throughput evidence: it ended `status=partial`, `throughput_valid=false`, at `676/75/751` because Free batch 2 scanned `151` pending and `49` cached items. Those `49` cache hits were written by the concurrent Pro lane through the shared transcript cache. The lane harness now assigns a separate `YTIS_TRANSCRIPT_CACHE_DB_PATH` under each lane output root, matching the existing batch-status isolation. Treat run06 as live validation of the duplicate-add fix and as evidence for the cache-isolation harness fix, not as a VPH result.

`fresh_state_3plus3_extract_schema_source_age_cadence_local_retry_projection_run07_current` activated that per-lane cache isolation and completed a valid smoke at `2543.94` combined VPH with `764/36/800`, but soak was invalidated in Pro batch 2 by a different `nlm_batch_source_mapping_failed` event. A cadence worker retained `12` prior-window sources, added a `38`-source window to reach `50`, parsed `34/38` source IDs from add output, and found `4` exact title matches. The extractor correctly refused to order-map the 38 current videos against all 50 visible sources. The follow-up fix records source IDs observed by the previous extraction and restricts both exact title matching and count-aligned order fallback to the IDs newly observed in the current cadence window; fresh-notebook creation clears that baseline. Partial add-output IDs remain fail-closed and are not positionally guessed.

`fresh_state_3plus3_extract_schema_source_age_cadence_local_retry_projection_run08_current` is the valid post-fix rerun and closes the local-retry projection branch as negative. The guarded `3+3` sequence completed with clean pre-run browser health, no source-mapping/materialization/default-profile invalidators, and the projection reason present in live logs. Smoke reached `1726.38` combined VPH with `659/141/800`; soak improved to `2512.72` with `770/30/800`, but remained far below `source_age_cadence_run01_current` at `3636.16` with `793/7/800`. Run08 soak accumulated `11570.936s` of content-fetch command time, `754.751s` of worker idle time, `source_ready_age_s_max=305.996`, and `24` source-age-cliff outcomes, versus run01's `2652.777s`, `268.162s`, `160.247`, and zero recorded source-age-cliff outcomes. The guard activates but does not restore the throughput ceiling; do not promote it or run another same-shape projection rerun without a new mechanism that reduces command latency or source age directly.

The follow-up event-level command attribution packet `command_latency_attribution_packet_run01_vs_run08` makes that negative result discriminating rather than merely aggregate. Reconciliation covered `98.9%` of run01 command count / `99.1%` of elapsed command time and `97.6%` of run08 command count / `98.8%` of elapsed command time, so the event rows are strong enough for mechanism triage. From run01 to run08, attempt-1 command elapsed rose `2191.209 -> 6273.605` (`+4082.396s`) and retry command elapsed rose `437.802 -> 5155.639` (`+4717.837s`); the regression is therefore not just retry sleep or late retry scheduling. Projection activations were only `6` events, all on Pro worker-01 in batches 1 and 2, while major deltas also occurred on Pro workers 02/03 and Free batch 1 with no Free projection activations. The next code path should target old-window `nlm source content` command latency and source-age accumulation before retry scheduling: inspect batch/profile/status-specific `command_failed` tails, source-window age, source-list/materialization health, and NotebookLM command duration itself. Do not run another live projection, first-window-cap, readiness-probe, or same-shape benchmark until that offline command-latency mechanism is narrowed or a code change exists.

The fetch-completion detail extension on the same packet confirms the old-window shape from the existing raw logs rather than requiring a new run. Run01 has `848` fetch-completed events grouped into `40` rows and run08 has `938` events grouped into `44` rows, with zero malformed rows after parsing nested payloads and deriving worker IDs from NotebookLM profiles. The top positive fetch-completion deltas are all `command_failed`: Pro batch 1 worker-02 retry `+1418.78s`, Free batch 1 worker-02 retry `+1229.12s`, Pro batch 1 worker-03 retry `+1130.36s`, Pro batch 1 worker-01 primary `+893.70s`, and Free batch 1 worker-01 primary `+715.55s`. The run08 top retry failures have source-ready ages around `239s`-`270s` and max command times up to `154.094s`; primary failures also show projected retry-ready ages around `200s`-`225s` with source-list probe cost attached to the primary pass. This points to already-aged NotebookLM source-content commands spending long wall time and then failing, not to the projection guard itself. Next offline work should compare the source-window/materialization timeline for those exact batch 1 workers and determine whether the aged commands share notebook state, source-count/window history, profile/browser state, or source-list validation behavior before designing a live code change.

The source-window timeline extension now produces that comparison from the same raw event logs in `.logs/sharded_lane_series/source_content_timeline_packet_run01_vs_run08.{md,json}` without another benchmark. Run01 contributes `3656` timeline rows and run08 contributes `4172`; run08 increases fetch-completed outcomes from `1654 -> 1916`, command failures from `74 -> 335`, source-age-cliff outcomes from `0 -> 156`, retry-queued fetches from `38 -> 186`, and dead-notebook recovery events from `16 -> 36`. The regression is broad across all six NotebookLM profiles rather than isolated to one worker: run08 Free worker-01 is worst (`ready=146`, `command_failed=70`, `source_age_cliff=48`, `max_source_ready_age_s=328.491`, `max_source_list_probe_s=120.002`), but Pro workers 01/02/03 also show large command-failure and age-cliff growth. This strengthens the conclusion that the next change should reduce source-window age accumulation or rotate notebooks before old-window fetches reach the cliff; it does not justify another local-retry-projection rerun.
...
The follow-up command-latency attribution packet `command_latency_attribution_packet_control_run07_vs_rotation180_run02_smoke.{md,json}` makes the negative smoke discriminating. Compared with the control smoke (`3291.38` VPH, `744/56/800`), rotation-180 smoke dropped to `2545.69` VPH and raised command elapsed from `9021.614s` to `14680.562s`. The regression is concentrated in batch 1: Free batch 1 carried `71` source-age-cliff outcomes, `24` command failures, `6616.767s` of content-fetch command time, `243.736s` of source-list probe time, and max source age `352.132s`; Pro batch 1 carried `30` source-age-cliff outcomes, `29` command failures, `7328.225s` of command time, `183.584s` of source-list probe time, and max source age `394.782s`. Batch 2 stayed comparatively healthy on both lanes (`198/2/200`, max age below `73s`). This supports the mechanism that rotation-at-window-boundary is too late once batch-1 source-content commands have already aged into the cliff. Do not tune first-window cap or local-retry projection from this result; both branches already have negative evidence. The next code path needs to reduce old-window `nlm source content` command latency/source-age accumulation directly or add first-class source-window timeline evidence before another live benchmark.

The source-window packet `source_content_timeline_packet_control_run07_vs_rotation180_run02_smoke.{md,json}` adds the missing first-class timeline evidence. Rotation-180 smoke parsed `18` term files, `6246` events, and `1921` timeline rows, with `16` source-age cadence windows completed and `6` age-guard rotations requested for `source_age_cliff`. The guard was active, but the failed batch-1 fetches were already old: Pro batch 1 had source-ready-age failed p50/max `231.725s/394.782s`, `30` actual cliffs, `29` projected cliffs, and `28` retry drains skipped for projected cliff; Free batch 1 had failed p50/max `226.983s/352.132s`, `71` actual cliffs, `24` projected cliffs, and `31` retry drains skipped for projected cliff. Batch 2 in both lanes had zero actual/projected cliffs and command p95 below `4s`. This closes the rotation-timing hypothesis as "active but too late for batch-1 old-window command pressure"; the next mechanism should make source-content commands avoid reaching the old-window state, not merely rotate after the window boundary.

The next code-path candidate is now available behind `YTIS_NLM_SOURCE_CONTENT_PRIMARY_COMMAND_AGE_PROJECTION_S`, defaulting to `0` so existing source-content behavior and the `source_age_cadence_run01_current` leader remain unchanged. When enabled, `NLMBatchIngestor.extract_transcripts()` checks the current source-ready age plus the configured primary-command projection before launching `nlm source content`; if that projected completion age reaches `YTIS_NLM_SOURCE_AGE_CLIFF_S`, it fails fast as `source_age_cliff` with `retry_queue_skipped_reason=projected_primary_command_age_cliff` instead of spending wall time on an old-window command expected to cross the cliff. There is also an opt-in margin knob, `YTIS_NLM_SOURCE_CONTENT_PRIMARY_COMMAND_AGE_MARGIN_S`, which defaults to `0` and makes the same preflight check use a margin-adjusted projected completion age; the fetch-completed attribution now records both the raw and margin-adjusted projections so reducer output can separate conservative gating from actual command-latency changes. Regression coverage: `tests/test_nlm_batch.py::TestNotebookCapRotation::test_source_content_fetch_skips_when_primary_command_projection_hits_age_cliff` and `tests/test_nlm_batch.py::TestNotebookCapRotation::test_source_content_fetch_skips_when_primary_command_projection_margin_hits_age_cliff`. Treat this as a narrow opt-in probe knob, not a default throughput fix, until a live run proves it reduces batch-1 old-window command pressure without lowering VPH. The current smoke summaries for `run02` and `run03` expose only aggregate cliff counts and max ready ages, not row-level projected-completion-age distributions, so the first live margin probe should use the already-tested 20s scale and be interpreted as a measurement run, not a proven improvement.

`fresh_state_3plus3_extract_schema_primary_command_projection_60_run02_current` is the first valid live probe of that knob after the default-profile stop-race fix. It completed `status=ok`, `throughput_valid=true`, `worker_shape_signature=3+3`, and `run_environment_label=home_300mb`; pre-run browser health was degraded by unrelated Chrome load (`unexpected_process_count=47`) but `detected_default=0`, and post-run hygiene was clean. Smoke was not a promotion-quality result: `2788.30` combined hot-path VPH on `728/72/800`, with `source_age_cliff=67`, `command_failed=49`, and `content_fetch_command_elapsed_s_total=9112.444`. The projection guard did fire in smoke: `nlm_batch_source_content_fetch_completed` rows recorded `primary_command_age_projection_s=60.0`, `projected_primary_command_completion_age_s` above the `200s` cliff, `retry_queue_skipped_reason=projected_primary_command_age_cliff`, and `attempts=0`. Soak was much stronger: `3788.53` combined hot-path VPH on `794/6/800`, Pro `397/3` at `2160.34` VPH, Free `397/3` at `2098.77` VPH, no aggregate `source_age_cliff`, `content_fetch_command_elapsed_s_total=4685.822`, and `worker_idle_wait_s_total=292.842`. Interpretation: projection-60 is a promising but mixed branch; it can produce a strong warmed soak, but the first smoke still enters the projected old-window state. Repeat before promotion, and be explicit about whether the goal is smoke-fail-fast diagnostics or warmed-state soak throughput.
...
The follow-up 20s margin measurement run (`fresh_state_3plus3_extract_schema_primary_command_projection_60_margin20_run03_current`) improved smoke to `2897.07` combined hot-path VPH on `705/95/800`, with `source_age_cliff=92`, `command_failed=28`, and the same `blocked_before_soak` outcome because the smoke floor still failed. Treat that result as measurement-only: it shows the margin knob can move the smoke shape, but it does not yet tell us whether a smaller margin, a larger margin, or a different projection constant would actually solve the batch-1 old-window pressure without creating a new cliff wave. Do not change primary-fetch gate semantics to retry-only; that branch is rejected. The next step was offline row-level attribution first, and that packet now exists.

The row-level packet is `P:\\packages\\yt-is\\.logs\\sharded_lane_series\\primary_command_age_projection_packet_projection60_vs_margin20_run03_smoke.{md,json}`. It compares the projection-60 control smoke against the margin20 smoke at fetch-event granularity. The packet shows the margin20 run materially reduced actual source-age cliffs (`340 -> 92`) and projected source-age cliffs (`270 -> 70`), raised ready rows (`457 -> 697`), and reduced command-failed rows (`186 -> 92`). Pro lane remaining cliffs are now all projection-induced, while Free lane's residual actual cliffs are retry-drain `drain_projected_source_age_cliff` rows in batch 1. That means margin20 is a useful guard, not a validated final fix.

Action status for the proposal that followed that result:

- done: keep the primary-fetch projection gate intact.
- done: update the plan and registry so `projection-60` and `margin20` are recorded as negative / measurement evidence, not a validated fix.
- done: rewrite the proposal into a falsifiable experiment packet and do offline row-level attribution first.
- done: generate the row-level projection packet for projection-60 versus margin20.
- deferred: choose the next narrow follow-up hypothesis after reviewing the Free-lane retry-drain residual cliffs; the current evidence points away from another same-shape smoke.

### Falsifiable experiment packet

- Hypothesis: margin20 is a useful guard but not the final fix; the residual Free-lane cliffs are retry-drain source-age cliffs, so the next lever is retry-drain or source-age accumulation in Free rather than more primary-command projection tuning.
- Why this is the next test: the row-level packet now discriminates the guard effect, and the remaining non-projection cliffs are specifically `drain_projected_source_age_cliff` rows in Free batch 1, which is a different failure path from the Pro lane.
- Success metrics: the next packet or probe reduces Free-lane retry-drain cliffs, keeps Pro from regressing into actual source-age cliffs, and preserves or improves combined VPH without reintroducing a new cliff wave.
- Failure metrics: the follow-up reverts to the old same-shape pattern, raises actual cliffs, or only shifts projection cliffs without reducing the residual Free-lane retry-drain failures.
- Falsifier: if a narrow Free-lane follow-up shows the retry-drain cliffs are not source-age-pressure driven, then the next lever should switch away from projection and retry-drain tuning entirely.
- Do not run next: another same-shape live projection smoke; use a changed hypothesis focused on Free-lane retry-drain/source-age accumulation or a nearby margin sweep only if it is explicitly justified.
