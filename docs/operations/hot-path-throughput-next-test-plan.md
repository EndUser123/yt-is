...
- `nlm_batch` now retries the source-count probe once after an auth failure before classifying the notebook as dead. This hardening is meant to reduce false `source_count_probe_failed` invalidations on stale worker-state paths, but it is a code-path fix rather than a fresh throughput datapoint until a live rerun confirms it.
