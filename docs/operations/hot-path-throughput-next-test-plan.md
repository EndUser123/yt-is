...
- **Candidate 6 per-attempt telemetry is live-proven (2026-07-01, run02).** The 11-field `nlm_batch_source_content_fetch_completed` contract validated on `candidate6_telemetry_validation_run02_current` (Signals 1/2/3 PASS on 1070 events; source-age-cliff `in_progress` leak fixed 169 → 0; per-attempt reconciliation = 0.0000 across 660 events = zero overhead). The measured distribution ranks the retry-tail Candidates 1-5 as minority levers — `primary_batch_wait_time_s` (median 57.5s) dominates the primary command loop (median 1.4s) by ~40× per source, so next effort targets primary-materialization, not the retry tail. See [`.logs/sharded_lane_series/candidate6_mechanism_ranking_after_run02.md`](P://packages/yt-is/.logs/sharded_lane_series/candidate6_mechanism_ranking_after_run02.md). Do not open a Candidate 1-5 live benchmark on this evidence; the retry path is too small a contributor to be cost-justified until a primary-materialization lever exists. Analyzer: `python scripts/analyze_candidate6_smoke.py --run-root <run02>`.
