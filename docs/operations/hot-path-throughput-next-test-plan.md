...
- Next move after this attribution: pivot back to the stronger age-capped source-readiness branch. The `fresh_state_3plus3_worker_balance_ab_pro0213` confirmation repeat already exists as run06 and regressed materially, so do not run another worker-balance confirmation unless worker-profile assignment, cleanup, or `nlm` fallback behavior changes.
