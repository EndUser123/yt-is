...
- Next move after this attribution: if you want another same-shape branch, repeat the `fresh_state_3plus3_worker_balance_ab_pro0213` profile-order candidate once more to confirm the median; otherwise pivot back to the stronger age-capped source-readiness branch. Do not change cadence thresholds again before using this split.
