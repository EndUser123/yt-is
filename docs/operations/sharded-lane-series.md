...
Lane configs may carry an optional `env` object for run-specific overrides; the sharded lane runner merges those overrides before setting the fixed lane identity and auth fields. Use that for benchmark-universe flags like the cadence toggle and thresholds.
Each lane's `lane_process.json` now also records an `env_snapshot` with the run-environment label, cadence toggle and thresholds, and worker-auth transport flag so reviewers can verify which benchmark universe was actually executed.
...
```powershell
$proRoot = 'P://.data/yt-is/browser/notebooklm-pro'
$proPort = 18870
...
```powershell
$freeRoot = 'P://.data/yt-is/browser/notebooklm-free'
$freePort = 18871
$chrome = 'C:\Program Files\Google\Chrome\Application\chrome.exe'
