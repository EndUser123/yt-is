...
- `csf-sharded-lane-sequence` runs an objective pre-run browser health gate after doctor and before smoke. It writes `<run-root>/browser_health.json`, reaps transient shared default NotebookLM Chrome profile leaks when the process matches the shared default-profile predicate and is clearly the NotebookLM session, keeps sampling through the settle window, and records the result in the top-level summary as `pre_run_browser_health`.
