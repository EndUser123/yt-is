...
# --- restored v1 bootstrap API (sibling refactor in flight) -------------
# The v1.5 page refactor removed evidence_cluster_inventory and
# hydrate_evidence_clusters, but concept_discovery.py,
# contract_v2_bakeoff.py, and the eval_* scripts still import them, which
# broke collection for the whole package suite. Restored verbatim from
# HEAD 83023f78 (with _catalog_at replacing the old _catalog signature).
# The owning lane should fold this block into the refactor or remove it
# when callers migrate to evidence_clusters().
def _catalog_at(catalog_path=None) -> sqlite3.Connection:
...
def evidence_cluster_inventory(min_member_count: int = 40,
    ...
    """
    # "Dashboard top-N stays allowed; inference bootstrap top-N is not."
    with closing(_catalog_at(catalog_path)) as c:
